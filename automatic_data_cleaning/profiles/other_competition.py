...
from rules.common.patterns import KEYWORDS_RE
...
from rules.other_competition.abstract import (
    abstract_text,
...
from rules.other_competition.body import extract_body_lines
...
def _is_four_digit_style(paper_id: str) -> bool:
    return bool(re.match(r"^\d{4}[.\s]", paper_id))


class OtherCompetitionProfile:
    ...
    def clean(self, raw_md: str, *, paper_id: str, separator: str) -> CleanResult:
        ...
        if _is_four_digit_style(paper_id) or "承诺书" in raw_md[:3000]:
            lines = strip_front_matter(lines)

        lines = strip_english_abstract_block(lines)
        ...
        if err:
            return CleanResult(
                ...
                abstract=abstract,
                body=body,
                meta={"paper_id": paper_id, "four_digit_style": _is_four_digit_style(paper_id)},
            )
